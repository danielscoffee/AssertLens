...
- A TypeSafe API key for live Jev reviews. Help and dry runs need no key.
...
Then run a trusted check before asking Jev:
...
:::warning Trust the command
Local checks are not sandboxed. Run only commands and code you trust. Use
`--head` for untrusted PR source; it reads committed Git data and refuses commands.
:::
